...
const FormControl = ({ darkMode }) => {
  ...
  const getTemplates = async () => {
    try {
      const res = await axios.get(
        "https://cervical.praispranav.com/day-template/get-template",
        {
  ...
  const getFormControl = async () => {
    try {
      const res = await axios.get(
        "https://cervical.praispranav.com/day-template/get-form-control",
        {
          headers: { "x-api-key": apiKey },
        }
      );
  ...
  const handleSave = async () => {
    const body = {
      ...
      eventRadioBtns: coordinateOptions
        .filter((opt) => opt.show)
        .map((opt) => opt.value),
    };
  ...
  return (
    <div className={`card card-style shadow-sm p-3 ${themeCard}`}>
      ...
      <div className="d-flex mt-3">
        ...
        />
        <button className="btn btn-secondary" onClick={addNewField}>
          Add
        </button>
      </div>

      <hr />
      <h5>Select a Event:</h5>
